...
Out of scope for Step 10. The parameter-count axis of the frontier is measured here (Tables 1 and 6); wall-clock latency on edge hardware is Step 11's deliverable.
...
3. **Latency measurement (RQ4 / Step 11)** — the Pareto frontier is currently parameter-count only, and the thesis's edge-deployment framing needs wall-clock evidence on real hardware.
